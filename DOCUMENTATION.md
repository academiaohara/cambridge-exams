...
|---|---|
| 3.3.3 | Rediseño del `exercise-footer` con UI/UX estilo Duolingo: barra de acciones en 3 zonas (secundarias a la izquierda, toggles en el centro, acción primaria a la derecha), botón Check gris "idle" que se ilumina en verde al responder (`ExerciseHandlers.syncFooterReadyState`), banner de feedback con icono, mensaje, chip de puntuación y tinte verde/naranja/rojo tras corregir (`ExerciseHandlers.syncFooterState`), Check → Continue verde tras la corrección y atajo de teclado Enter (corregir / continuar) en modo práctica. Estructura compartida vía `ExerciseRenderer.buildExerciseFooterShell` (también usada por el footer de carga en `exercise.js`); sincronización enganchada a `Timer.updateScoreDisplay`. |
| 3.3.2 | Rediseño del perfil estilo Duolingo: banner degradado con avatar solapado y pill de nivel, tarjetas de estadísticas horizontales con iconos rellenos, lista única de progreso con separadores y botón de sign out con borde 3D. En móvil, todo lo que se abre desde el perfil se presenta como bottom sheet (racha, evolución de notas, selector de idioma —ahora lista scrolleable—, calculadora con fondo atenuado y cierre con Escape). La página de planes incluye la bottom nav en móvil, resetea el scroll y su botón Atrás vuelve al perfil si se abrió desde ahí. |
