...
(function() {
  ...
  window.FastExercises = {
    ...
    _setVocabLessonFocus: function(active) {
      ...
      if (center) {
        center.classList.toggle('course-center--lesson-focus', !!active);
        center.classList.remove('fe-vocab-lesson-focus');
      }
    },
    ...
    _ensureVocabCenterSection: function() {
      ...
      if (!centerSection) {
        ...
        content.innerHTML =
          '<div class="dashboard-layout dashboard-layout--crossword-scroll dashboard-layout--lesson-focus">' +
            ...
            '<div class="dashboard-center dashboard-center--course course-center--lesson-focus fe-vocab-sp-center">' +
              '<div class="fe-section fe-section--merged-path fe-section--vocab-exercise course-unit-content" id="feCenterSection"></div>' +
            '</div>' +
            (typeof Dashboard !== 'undefined' && Dashboard._renderSidebarShell
    ...
    _restoreVocabMapChrome: function() {
      var center = document.querySelector('.dashboard-center--course-vocab');
      if (!center) return;
    ...
    _beginVocabPracticeSession: function(container, opts) {
      var session = window.FastExercisesVocabSession;
      if (!session) return null;
      if (session.destroy) session.destroy();
      this._hideVocabMapChrome();
      if (opts && opts.passive) {
        return session.startPassiveSession(this, container, opts) ? session.getScreenMount() : null;
      }
      return null;
    },

    _getVocabPointInstruction: function(point, extraInstruction) {
      return this._getPointInstruction(point, extraInstruction) || '';
    },

    _runVocabQuizSession: function(container, exercises, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel) {
      var self = this;
      var session = window.FastExercisesVocabSession;
      if (!session || !exercises || !exercises.length) return false;
      ...
      return session.startQuizSession(self, container, {
    ...
    _runVocabPassiveSession: function(container, contentHtml, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel, onContinue, instructionOverride) {
      ...
      return session.startPassiveSession(self, container, {
        categoryId: catMeta.id,
        catMeta: catMeta,
        levelId: levelId,
        lessonId: lessonId,
        ...
        lessonTitle: lessonTitle,
        pointLabel: pointLabel,
        sessionCorrect: pointIndex,
        sessionTotal: lessonPoints ? lessonPoints.length : 1,
        contentHtml: contentHtml,
        instruction: instruction,
        onContinue: function() {
          if (onContinue) onContinue();
          else self._completeAndNext(catMeta.id, levelId, lessonId, pointIndex);
        }
      });
    },

    _clearVocabPracticeSession: function() {
      if (window.FastExercisesVocabSession && window.FastExercisesVocabSession.destroy) {
        window.FastExercisesVocabSession.destroy();
      }
      if (window.FastExercisesVocabTheory && window.FastExercisesVocabTheory.destroy) {
        window.FastExercisesVocabTheory.destroy();
      }
      this._restoreVocabMapChrome();
    },
    ...
    _isTheoryPointType: function(pointType) {
      return pointType === 'pv-gallery' || pointType === 'id-gallery' || pointType === 'wf-explanation';
    },

    _getTheoryPointIndex: function(points) {
      if (!points) return -1;
      for (var i = 0; i < points.length; i++) {
        if (this._isTheoryPointType(points[i].type)) return i;
      }
      return -1;
    },

    _getCourseExerciseLabel: function(idx) {
      if (typeof DashboardNav !== 'undefined' && DashboardNav._getCourseExerciseLabel) {
        return DashboardNav._getCourseExerciseLabel(idx);
      }
      var n = idx + 1;
      var label = '';
      while (n > 0) {
    ...
    _buildMapExerciseCellHtml: function(catMeta, levelId, lessonId, point, pi, options) {
      options = options || {};
      ...
      if (isDone) cellClass += ' course-exercise-cell--done';
      else if (!isAccessible) cellClass += ' course-exercise-cell--locked';
      else cellClass += ' course-exercise-cell--pending';
      if (options.currentKey === pointKey && isAccessible && !isDone) {
        cellClass += ' course-exercise-cell--current';
      }
      var label = options.label || this._getCourseExerciseLabel(options.exerciseIdx || 0);
      var onclick = (isAccessible || isDone)
        ? 'onclick="FastExercises.openPoint(\'' + catMeta.id + '\', \'' + levelId + '\', \'' + lessonId + '\', ' + pi + ')"'
        : '';
      return '<button type="button" class="' + cellClass + '" ' + onclick +
        ' title="' + this._escapeHTML(point.label || '') + '"' +
        (!isAccessible && !isDone ? ' disabled' : '') + '>' +
        '<span class="course-exercise-cell-label">' + label + '</span>' +
      '</button>';
    },

    _scrollVocabPathToCurrent: function() {
      requestAnimationFrame(function() {
        var scrollEl = document.getElementById('feCenterScroll');
        var current = document.querySelector('.course-etapa-map .course-exercise-cell--current');
        if (!current) return;
        if (scrollEl) {
    ...
    openCategory: async function(categoryId) {
      ...
      // ── LEGEND (legacy map only; course vocab uses learning-style etapa map) ──
      var legendHtml = '';
      if (!isCourseVocab && categoryId === 'phrasal-verbs') {
        ...
        '</div>';
      } else if (!isCourseVocab && categoryId === 'word-formation') {
        legendHtml = '<div class="fe-map-legend fe-map-legend-top">' +
      ...
      content.innerHTML =
        '<div class="dashboard-layout' + (isCourseVocab ? ' dashboard-layout--crossword-scroll' : '') + '">' +
          (typeof Dashboard !== 'undefined' && Dashboard._renderSidebarShell
            ? Dashboard._renderSidebarShell('left', 'dashboardLeftSidebarShell', 'dashboardLeftSidebar', leftSidebarContent)
            : '<div class="dashboard-left-sidebar">' + leftSidebarContent + '</div>') +
          '<div class="dashboard-center' + (isCourseVocab ? ' dashboard-center--crossword dashboard-center--mobile-hub dashboard-center--course-vocab dashboard-center--learning-stage' : '') + '">' +
      ...
      var self = this;
      if (isCourseVocab) {
        requestAnimationFrame(function() { self._scrollVocabPathToCurrent(); });
      } else {
    ...
    /** B1→C1 continuous path for course vocabulary categories (learning-style etapa map). */
    _buildMergedProgressionMap: function(catMeta, data) {
      ...
      var html = '<div class="course-etapa-map" data-section="vocabulary" data-category="' + catMeta.id + '">';
      html += '<div class="course-etapa-path">';
      ...
      for (var mi = 0; mi < merged.length; mi++) {
        ...
        var theoryIdx = self._getTheoryPointIndex(lesson.points);
        var headerTitle = 'Lesson ' + (mi + 1) + ' · ' + levelId + ' · ' + (lesson.title || lesson.id);

        html += '<div class="course-etapa-header course-etapa-header--unit course-etapa-header--vocab">';
        html += '<div class="course-etapa-header-text">';
        html += '<div class="course-etapa-header-title">' + self._escapeHTML(headerTitle) + '</div>';
        html += '</div>';
        if (theoryIdx >= 0 && lessonUnlocked) {
          html += '<button type="button" class="course-etapa-header-guide" onclick="event.stopPropagation();FastExercises.openVocabGuide(\'' +
            catMeta.id + '\',\'' + levelId + '\',\'' + lesson.id + '\')">' +
        ...
        if (lesson.points) {
          ...
          for (var pi = 0; pi < lesson.points.length; pi++) {
            var point = lesson.points[pi];
            if (self._isTheoryPointType(point.type)) continue;
            ...
            html += self._buildMapExerciseCellHtml(catMeta, levelId, lesson.id, point, pi, {
              isDone: isDone,
              isAccessible: isAccessible,
              currentKey: currentKey,
              exerciseIdx: exerciseIdx,
              label: self._getCourseExerciseLabel(exerciseIdx)
            });
            exerciseIdx++;
          }
        }
    ...
    openVocabGuide: async function(categoryId, levelId, lessonId) {
      if (!this._isCourseVocabCategory(categoryId)) return;
      var self = this;
      var container = this._ensureVocabCenterSection();
      if (!container) return;
      ...
      if (catData && catData.levels) {
        for (var i = 0; i < catData.levels.length; i++) {
          if (catData.levels[i].id !== levelId || !catData.levels[i].lessons) continue;
          for (var j = 0; j < catData.levels[i].lessons.length; j++) {
            var lsn = catData.levels[i].lessons[j];
            if (lsn.id !== lessonId) continue;
            lessonTitle = lsn.title || lessonId;
            theoryIdx = self._getTheoryPointIndex(lsn.points);
            break;
          }
        }
      }

      if (theoryIdx < 0 || !self._canAccessPoint(categoryId, levelId, lessonId, theoryIdx, catData)) {
        await self._waitLoading(loadingStart);
        self.openCategory(categoryId);
        return;
      }

      var theoryModule = window.FastExercisesVocabTheory;
      if (!theoryModule || !theoryModule.buildTheoryUnit || !theoryModule.startTheorySession) {
        await self._waitLoading(loadingStart);
        self.openCategory(categoryId);
        return;
      }

      var unitData = theoryModule.buildTheoryUnit(categoryId, lessonData || {}, lessonTitle);
      var cards = unitData.theory && unitData.theory.cards;
      if (!cards || !cards.length) {
        await self._waitLoading(loadingStart);
        self._markPointComplete(categoryId, levelId, lessonId, theoryIdx);
        self.openCategory(categoryId);
        return;
      }
      ...
      self._setVocabLessonFocus(true);

      var guideState = { view: 'fastExerciseCategory', categoryId: categoryId };
      history.pushState(guideState, '', Router.stateToPath(guideState));
    },
    ...
    openPoint: async function(categoryId, levelId, lessonId, pointIndex) {
      ...
      if (isCourseVocab && this._isTheoryPointType(pointType)) {
        this.openVocabGuide(categoryId, levelId, lessonId);
        return;
      }
      ...
      if (idTypes.indexOf(pointType) !== -1) {
        if (pointType === 'id-trophy') {
          this._markPointComplete(categoryId, levelId, lessonId, pointIndex);
          var trophyHtml =
            '<div class="fe-point-card fe-vocab-sp-trophy">' +
            ...
            '</div>';
          if (self._runVocabPassiveSession(container, trophyHtml, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel, function() {
            self.openCategory(categoryId);
          })) {
            self._finalizeCourseVocabPoint(container, categoryId, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel);
            return;
          }
          container.innerHTML = '<div class="fe-point-view">' + trophyHtml +
            '<button class="fe-point-next-btn" onclick="FastExercises.openCategory(\'' + categoryId + '\')" style="background:' + (catMeta ? catMeta.color : '#f59e0b') + '">' +
              _backButtonContent('Map') + '</button></div>';
          self._finalizeCourseVocabPoint(container, categoryId, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel);
          return;
        }
    ...
    _renderIdGallery: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var galleryHtmlId =
        '<div class="pv-gallery-single-wrap fe-vocab-sp-gallery">' +
          '<div class="pv-gallery-cards-area" id="id-gallery-cards">' + cardsHtml + '</div>' +
          '<div class="pv-gallery-nav-col" id="id-gallery-nav">' + dotsHtml + '</div>' +
        '</div>';

      var pointId = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabelId = pointId ? (pointId.label || '') : '';
      if (self._runVocabPassiveSession(container, galleryHtmlId, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabelId)) {
        var cardsAreaId = document.getElementById('id-gallery-cards');
        if (cardsAreaId) {
          cardsAreaId.addEventListener('wheel', function(e) {
            e.preventDefault();
          ...
          self._attachHorizontalSwipe(cardsAreaId, function() {
            FastExercises._idGalleryGoTo((FastExercises._idGalleryCurrentIdx || 0) - 1);
          }, function() {
            FastExercises._idGalleryGoTo((FastExercises._idGalleryCurrentIdx || 0) + 1);
          });
        }
        self._idGalleryCurrentIdx = 0;
        self._idGalleryTotal = idioms.length;
        return;
      }
    ...
    _renderIdFillIn: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      if (self._runVocabQuizSession(container, exercises, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel)) {
        return;
      }
    ...
    _renderIdQuiz: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var pointQz = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabelQz = pointQz ? (pointQz.label || '') : '';
      if (self._runVocabQuizSession(container, questions, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabelQz)) {
        return;
      }
    ...
    _renderPvGallery: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var galleryHtml =
        '<div class="pv-gallery-single-wrap fe-vocab-sp-gallery">' +
          '<div class="pv-gallery-cards-area" id="pv-gallery-cards">' + cardsHtml + '</div>' +
          '<div class="pv-gallery-nav-col" id="pv-gallery-nav">' + dotsHtml + '</div>' +
        '</div>';

      var point = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabel = point ? (point.label || '') : '';
      if (self._runVocabPassiveSession(container, galleryHtml, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabel)) {
        ...
        if (cardsArea) {
          ...
          self._attachHorizontalSwipe(cardsArea, function() {
            FastExercises._pvGalleryGoTo((FastExercises._pvGalleryCurrentIdx || 0) - 1);
          }, function() {
            FastExercises._pvGalleryGoTo((FastExercises._pvGalleryCurrentIdx || 0) + 1);
          });
        }
        self._pvGalleryCurrentIdx = 0;
        self._pvGalleryTotal = pvs.length;
        return;
      }
      ...
      var cardsAreaLegacy = document.getElementById('pv-gallery-cards');
      if (cardsAreaLegacy) {
        cardsAreaLegacy.addEventListener('wheel', function(e) {
          e.preventDefault();
          var current = FastExercises._pvGalleryCurrentIdx || 0;
          if (e.deltaY > 0) FastExercises._pvGalleryGoTo(current + 1);
          else FastExercises._pvGalleryGoTo(current - 1);
        }, { passive: false });
        this._attachHorizontalSwipe(cardsAreaLegacy, function() {
          FastExercises._pvGalleryGoTo((FastExercises._pvGalleryCurrentIdx || 0) - 1);
        }, function() {
          FastExercises._pvGalleryGoTo((FastExercises._pvGalleryCurrentIdx || 0) + 1);
        });
    ...
    _renderWfExplanation: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var explanationHtml =
        '<div class="wf-explanation-wrap fe-vocab-sp-passive">' +
          (groups.length > 0 ? '<div class="wf-exp-panel">' + ruleHtml + groupsHtml + '</div>' : '') +
          (wordForms.length > 0 ?
      ...
      var pointWf = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabelWf = pointWf ? (pointWf.label || '') : '';
      if (self._runVocabPassiveSession(container, explanationHtml, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabelWf)) {
        var cardsAreaWf = document.getElementById('wf-gallery-cards');
        if (cardsAreaWf) {
          cardsAreaWf.addEventListener('wheel', function(e) {
            e.preventDefault();
          ...
          self._attachHorizontalSwipe(cardsAreaWf, function() {
            FastExercises._wfCardGoTo((FastExercises._wfCardCurrentIdx || 0) - 1);
          }, function() {
            FastExercises._wfCardGoTo((FastExercises._wfCardCurrentIdx || 0) + 1);
          });
        }
        self._wfCardCurrentIdx = 0;
        self._wfCardTotal = wordForms.length;
        return;
      }
    ...
    _renderWfMultipleChoice: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var pointMc = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabelMc = pointMc ? (pointMc.label || '') : '';
      if (self._runVocabQuizSession(container, normalizedMc, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabelMc)) {
        return;
      }
    ...
    _renderWfTransform: function(container, lessonData, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints) {
      ...
      var normalizedTr = exercises.map(function(ex) {
        return {
          type: 'transform',
      ...
      var pointTr = (lessonPoints && lessonPoints[pointIndex]) ? lessonPoints[pointIndex] : null;
      var pointLabelTr = pointTr ? (pointTr.label || '') : '';
      if (self._runVocabQuizSession(container, normalizedTr, catMeta, levelId, lessonId, lessonTitle, pointIndex, lessonPoints, pointLabelTr)) {
        return;
      }
    ...
    _wdlFormatLocalDate: function(d) {
    ...
    _computeWlStats: function(levelId) {
      ...
      var formatDate = FastExercises._wdlFormatLocalDate;
      var today = new Date();
      var currentStreak = 0;
    ...
    _wdlBuildSolvedHtml: function(state) {
      ...
      return '<div class="wdl-solved-card">' +
        ...
        '<div class="wdl-solved-title">Solved!</div>' +
        '<div class="wdl-solved-subtitle">You got it in ' + guessCount + ' ' + triesLabel + '</div>' +
        '<div class="wdl-solved-banner">' +
          '<span class="material-symbols-outlined" aria-hidden="true">celebration</span>' +
          '<span>' + bannerText + '</span>' +
        '</div>' +
        '<div class="wdl-solved-stats">' +
          '<div class="wdl-solved-stat"><div class="wdl-solved-stat-val">' + stats.played + '</div><div class="wdl-solved-stat-lbl">Played</div></div>' +
    ...
    _wdlBuildStatsDistHtml: function(stats) {
      var totalWins = stats.wins || 0;
      var maxCount = 0;
      var rows = [];
      var gi;
      for (gi = 0; gi < 6; gi++) {
        if (stats.distribution[gi] > maxCount) maxCount = stats.distribution[gi];
      }
      if (stats.fails > maxCount) maxCount = stats.fails;

      for (gi = 0; gi < 6; gi++) {
        var count = stats.distribution[gi];
        var pct = totalWins > 0 ? Math.round((count / totalWins) * 100) : 0;
        var barPct = maxCount > 0 ? Math.round((count / maxCount) * 100) : 0;
        rows.push(
          '<div class="wdl-dist-row">' +
            '<span class="wdl-dist-num">' + (gi + 1) + '</span>' +
            '<div class="wdl-dist-bar-wrap"><div class="wdl-dist-bar" style="width:' + barPct + '%"></div></div>' +
            '<span class="wdl-dist-count">' + count + ' (' + pct + '%)</span>' +
          '</div>'
        );
      }
      var failPct = stats.played > 0 ? Math.round((stats.fails / stats.played) * 100) : 0;
      var failBarPct = maxCount > 0 ? Math.round((stats.fails / maxCount) * 100) : 0;
      rows.push(
        '<div class="wdl-dist-row">' +
          '<span class="wdl-dist-num">X</span>' +
          '<div class="wdl-dist-bar-wrap"><div class="wdl-dist-bar wdl-dist-bar--fail" style="width:' + failBarPct + '%"></div></div>' +
          '<span class="wdl-dist-count">' + stats.fails + ' (' + failPct + '%)</span>' +
        '</div>'
      );
      return rows.join('');
    },
    ...
    _renderStandaloneWordle: function(pageEl) {
      ...
      var gameOver = state.solved || state.guesses.length >= MAX_GUESSES;
      var msgHtml = state.solved
        ? FastExercises._wdlBuildSolvedHtml(state)
        : (state.guesses.length >= MAX_GUESSES
            ? FastExercises._wdlBuildFailedHtml(state)
            : '');
      ...
      var keyboardHtml = isMobile && !gameOver
      ...
      pageEl.innerHTML =
        '<div class="cw-wordle-duo' + (gameOver ? ' cw-wordle-duo--game-over' : '') + '">' +
