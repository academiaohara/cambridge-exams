...
function validateExercise(file, exercise) {
  ...
  (exercise.items || []).forEach(function(item, idx) {
    ...
    if (item.formatType === 'find_extra_word' || ft === 'find_extra_word') {
      if (!item.explanationContent && !item.explanation) {
        issues.push(warn(file, 'find_extra_word item ' + (item.id || idx) + ' missing explanationContent'));
      } else if (item.explanationContent && !item.explanationContent.whyCorrect) {
        issues.push(warn(file, 'find_extra_word item ' + (item.id || idx) + ' explanationContent missing whyCorrect'));
      }
      if (item.explanation) {
    ...
    if (item.formatType === 'crossword_clues' || ft === 'crossword_clues') {
      if (!item.explanationContent && !item.explanation) {
        issues.push(warn(file, 'crossword_clues item ' + (item.id || idx) + ' missing explanationContent'));
      } else if (item.explanationContent && !item.explanationContent.whyCorrect) {
        issues.push(warn(file, 'crossword_clues item ' + (item.id || idx) + ' explanationContent missing whyCorrect'));
      }
      if (item.explanation) {
    ...
    if (item.formatType === 'stative_sorting' || ft === 'stative_sorting') {
      if (!item.explanationContent && !item.explanation) {
        issues.push(warn(file, 'stative_sorting item ' + (item.id || idx) + ' missing explanationContent'));
      } else if (item.explanationContent && !item.explanationContent.whyCorrect) {
        issues.push(warn(file, 'stative_sorting item ' + (item.id || idx) + ' explanationContent missing whyCorrect'));
      }
      if (item.explanation) {
        issues.push(warn(file, 'stative_sorting item ' + (item.id || idx) + ' still uses legacy explanation string'));
      }
    }
    if (item.formatType === 'passage_error_hunt_single' || ft === 'passage_error_hunt_single' ||
        ft === 'passage_error_hunt_counter') {
      if (!item.explanationContent && !item.explanation) {
        issues.push(warn(file, 'passage_error_hunt item ' + (item.id || idx) + ' missing explanationContent'));
      } else if (item.explanationContent && !item.explanationContent.whyCorrect) {
        issues.push(warn(file, 'passage_error_hunt item ' + (item.id || idx) + ' explanationContent missing whyCorrect'));
      }
      if (item.explanation) {
