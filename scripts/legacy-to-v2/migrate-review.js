#!/usr/bin/env node
/**
 * Migrate legacy course reviews to Sune Play v2 schema.
 *
 * Usage:
 *   node scripts/legacy-to-v2/migrate-review.js --level C1
 *   node scripts/legacy-to-v2/migrate-review.js --input data/Course/B1/Review1.json --level B1
 *   node scripts/legacy-to-v2/migrate-review.js --all
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { buildReviewUnit } from './build-review.js';
...
function migrateFile(inputPath, level, outputPath) {
  ...
  var reviewMatch = path.basename(inputPath).match(/^Review(\d+)\.json$/i);
  var reviewNum = reviewMatch ? parseInt(reviewMatch[1], 10) : legacy.block;
  var result = buildReviewUnit(legacy, {
    level: level,
    reviewNum: reviewNum,
...
function main() {
  ...
  if (opts.all) {
    ...
    LEVELS.forEach(function(level) {
      ...
      fs.readdirSync(dir)
        .filter(function(name) { return /^Review\d+\.json$/i.test(name) && !name.endsWith('.v2.json'); })
        .sort()
