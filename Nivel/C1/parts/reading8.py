...
def get_ai_content(api_key, test_id):
    ...
    {{
      ...
      "content": {{
        "title": "Section title",
        "texts": {{
          "A": "### [Name, Role]\\nFirst-person account ~160 words. Embed [47]evidence[/47] markers.",
          "B": "### [Name, Role]\\n~160 words.",
          "C": "### [Name, Role]\\n~160 words.",
          "D": "### [Name, Role]\\n~160 words."
        }},
    ...
    2. 10 questions (47–56) phrased as participial clauses ("mentions doing X?", "describes feeling Y?").
    3. Each question must have EXACTLY ONE correct answer.
    4. Balance: Each person (A-D) should be the correct answer 2 or 3 times.
    5. Embed evidence markers [N]...[/N] in the text at the exact phrase that answers question N.
    6. Ensure the distractors are strong (other people might mention similar topics but not the specific detail asked).
...
def generate(test_id, output_path, api_key, json_only=True, *args, **kwargs):
    ...
    """
    json_file = output_path / f"test_{test_id}_reading8.json"
