...
def _expand_listening3(client, content, min_words=450):
    """Expands the interview audio_script without regenerating the full JSON."""
    audio_script = content.get("extracts", [{}])[0].get("audio_script", "")
    current_count = count_words(audio_script)
    expand_prompt = (
        f"The interview audio_script is too short ({current_count} words, minimum required: {min_words}). "
        f"Expand it by adding more detail and elaboration to existing exchanges. "
        f"Keep ALL existing answer markers (e.g. [15]...[/15]) exactly in place. "
        f"Do NOT add markers to the dialogue array. "
    ...
    if not audio_only:
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            if attempt <= 3:
                ...
                content = _expand_listening3(client, content, MIN_WORDS)
            audio_script = content.get("extracts", [{}])[0].get("audio_script", "")
            word_count = count_words(audio_script)
            if word_count >= MIN_WORDS:
                break
            if attempt < MAX_ATTEMPTS:
                print(f"⚠️ Intento {attempt}: Solo {word_count} palabras en el audio_script (mínimo {MIN_WORDS}). {'Regenerando' if attempt < 3 else 'Expandiendo'}...")
            else:
