...
def get_ai_content(api_key, test_id):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            "CRITICAL: Each person's text (A, B, C, D) MUST contain at least 100 words. "
            "Count every word before returning. If any text is under 100 words, expand it before outputting."
        )},
...
def _expand_reading8(client, data, min_per_text=95):
    """Expands person texts that are below min_per_text words."""
    texts = data.get("content", {}).get("texts", {})
    short = {k: v for k, v in texts.items() if count_words(v) < min_per_text}
    if not short:
        return data
    expand_prompt = (
        f"The following person texts are too short (each needs ≥{min_per_text} words). "
        f"Expand ONLY these texts in place, keeping the same voice, topic, and embedded evidence markers (e.g. [47]...[/47]). "
    ...
    try:
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            if attempt <= 3:
                ...
                data = _expand_reading8(client, data)
            texts = data.get("content", {}).get("texts", {})
